  ...
  useEffect(() => {
    const fetchDataForChart = async (chartNumber) => {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_APP_URL}/webapp/calculate-shift-efficiency/${startDate}/${endDate}/${chartNumber}`
      );
 ...
 useEffect(() => {
  const fetchAverageEfficiency = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_APP_URL}/webapp/api/avg/${startDate}/${endDate}`
      );
      const data = response.data;
